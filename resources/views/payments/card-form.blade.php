    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-md mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">
                {{ __('general.card_payment') }}
            </h2>
            
            <div class="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <h3 class="font-semibold text-blue-900 dark:text-blue-100">
                    {{ __('general.payment_details') }}
                </h3>
                <p class="text-sm text-blue-700 dark:text-blue-300">
                    {{ __('general.group') }}: {{ $group->name }}
                </p>
                <p class="text-sm text-blue-700 dark:text-blue-300">
                    {{ __('general.amount') }}: ₦{{ number_format($group->contribution_amount, 2) }}
                </p>
                <p class="text-sm text-blue-700 dark:text-blue-300">
                    {{ __('general.recipient') }}: {{ $group->currentTurnUser->name }}
                </p>
            </div>
            ...
            <form id="cardPaymentForm" action="{{ route('alat.card.initialize') }}" method="POST">
                ...
                <div class="flex space-x-4">
                    <button type="submit" 
                        ...
                        {{ __('general.pay_amount', ['amount' => '₦' . number_format($group->contribution_amount, 2)]) }}
                    </button>
